...
function registerRoutes() {
  ...
  app.get('/api/queue', (req, res) => {
    try {
      const items = queue.getPendingUrls();
    ...
    try {
      ...
      if (result.added > 0) {
        processQueue().catch((err) => {
          console.error('Background processing failed:', err.message || err);
        });
      }
  ...
  app.post('/api/queue/process', async (req, res) => {
    try {
      processQueue().catch((err) => {
        console.error('Manual processing failed:', err.message || err);
      });
      const pending = queue.getPendingUrls();
...
function startServer() {
  ...
  const server = app.listen(PORT, () => {
    ...
    console.log('Queue stored in links.txt (file-based, no Redis required).\n');
  });
